...
export const listframe = async (req, res) => {
  try {
    const frame = await Frame.find({}).limit(12).sort({ createdAt: -1 });
...
export const frameimg = async (req, res) => {
  try {
    const frame = await Frame.findById(req.params.frameId).select(
      "img"
    );
    if (
      frame.img &&
      Array.isArray(frame.img.url) &&
      frame.img.url.length > 0
    ) {
...
export const listtire = async (req, res) => {
  try {
    const tire = await Tire.find({}).limit(12).sort({ createdAt: -1 });
...
export const tireimg = async (req, res) => {
  try {
    const tire = await Tire.findById(req.params.tireId).select(
      "img"
    );
    if (
      tire.img &&
      Array.isArray(tire.img.url) &&
      tire.img.url.length > 0
    ) {
      // If img is an array of URLs and contains at least one URL, redirect the client to the first URL in the array
...
export const listgroupset = async (req, res) => {
  try {
    const groupset = await Groupset.find({}).limit(12).sort({ createdAt: -1 });
